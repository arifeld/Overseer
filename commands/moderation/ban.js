...
module.exports = class AddBan extends Command {
    constructor(client) {
        super(client, {
            ...
            group: "moderation",
            memberName: "ban",
            description: "Bans the user with a given length and reason. *You must add a ban length (or 0 for perm bans) or the reason will format wrong.*",
            guildOnly: true,
            format: "<user> [length (i.e. 1d) or 0 for perm] [reason]",
            argsType: "multiple",
            argsCount: 3,
            topPerm: "BAN_MEMBERS"
            /*
    ...
    async run(msg, args) {
        //const {user, length, tempreason} = args;
        ...
        if (!msg.member.hasPermission("BAN_MEMBERS")) {
            ...
            // update: we don't do anything now.
            return
        }
        ...
        if (guildMember !== undefined && msg.member == guildMember) {
            const errorEmbed = new RichEmbed()
        ...
        let reason = tempreason

        // We want to check if the mod has just skipped the ban duration.
        if (banTime == 0 && length !== "0") {
            // Change the reason.
            console.log(banTime)
            console.log(length)
            if (tempreason !== "None given") {
                reason = length + " " + tempreason
            } else {
                reason = length
            }
        }
            ...
            msg.embed(updateEmbed) // we don't want a new case to be made.
                .then(embedMsg => {
                    ...
                    const logEmbed = new RichEmbed()
                        ...
                        .setTimestamp(banTimestamp)
                        .setFooter("Case Number: " + previousBanInfo.case)
                    msg.guild.channels.get(botConfig.modLogChannel).send({
                        embed: logEmbed
                    })
                })
            ...
            .then(channel => {
                channel.send({
                    ...
                    .catch(err => {
                        ...
                        let discordBanMessage = "(CASE " + caseNumber + "): " + reason + " - " + banLength + " - " + (msg.member.nickname || msg.member.user.username)
                        if (discordBanMessage.length > 510){
                            discordBanMessage = "(CASE " + caseNumber + "): <REASON TOO LONG, CHECK MOD NOTES> - " + banLength + " - " + (msg.member.nickname || msg.member.user.username)
                        }

                        msg.guild.ban(user.id, {
                                reason: discordBanMessage
                            })
                            .then(console.log("User: " + user.id + " banned."))
